...
public:
    ...
    void send_goal(int target_number, double period)
    {
        ...
        options.result_callback = 
            std::bind(&CountUntilClientNode::goal_result_callback, this, std::placeholders::_1);
        options.goal_response_callback = 
            std::bind(&CountUntilClientNode::goal_response_callback, this, std::placeholders::_1);
...
private:
    ...
    void goal_result_callback(const CountUntilGoalHandle::WrappedResult &result)
    {
        ...
        int reched_number = result.result->reached_number;
        RCLCPP_INFO(this->get_logger(), "Result: %d", reched_number);
    }
